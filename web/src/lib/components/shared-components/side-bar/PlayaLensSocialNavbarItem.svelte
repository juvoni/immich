...
<div>
  <div class="relative flex items-center">
    <Link
      ...
      underline={false}
      class="flex w-full place-items-center gap-4 rounded-e-full py-3 ps-5 transition-[padding] delay-100 duration-100 hover:bg-subtle hover:text-primary"
    >
      <div class="relative flex w-full place-items-center gap-4">
        ...
        {#if showInbox || showReview}
          <span class="ms-auto flex items-center gap-1 pe-3" aria-label={badgeAria}>
            {#if showInbox}
              <span
                class="rounded-full bg-primary px-2 py-1 text-[11px] leading-none font-semibold text-light"
                title={inboxTitle}
              >
                {inboxLabel}
              </span>
            {/if}
            {#if showReview}
              <span class="size-2 rounded-full bg-danger" title={reviewLabel} aria-hidden={true}></span>
            {/if}
          </span>
        {/if}
      </div>
